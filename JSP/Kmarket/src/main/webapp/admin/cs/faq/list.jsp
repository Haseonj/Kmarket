...
<script>
	$(function(){
		...
		$('select[name=type1]').change(function(){
			...
			location.href = "/Kmarket/admin/cs/faq/list.do?cate1="+cate1+"&cate2=all&pg=1";
		...
		$('select[name=type2]').change(function(){
			...
			location.href = "/Kmarket/admin/cs/faq/list.do?cate1="+cate1+"&cate2="+cate2+"&pg=1";
                ...
                <article>
                    ...
                    <table>
                        ...
                        <c:forEach var="faq" items="${faq}">
	                        <tr>
	                            <td><input type="checkbox" name="chk" value="${faq.no}"></td>
	                            <td>${pageStartNum = pageStartNum - 1}</td>
	                            <td>${faq.c1Name}</td>
	                            <td>${faq.cate2}</td>
	                            <td class="title"><a href="/Kmarket/admin/cs/faq/view.do?cate1=${faq.cate1}&cate2=${faq.cate2}&no=${faq.no}&pg=${pg}">${faq.title}</a></td>
	                            <td>${faq.hit}</td>
	                            <td>${faq.rdate.substring(2, 10)}</td>
	                            <td>
	                                <a href="#" class="delete">[삭제]</a><br/>
	                                <a href="/Kmarket/admin/cs/faq/modify.do?cate1=${faq.cate1}&cate2=${faq.cate2}&no=${faq.no}&pg=${pg}">[수정]</a>
	                            </td>
	                        </tr>
                        </c:forEach>
                    </table>
                    <a href="#" class="chkDelete">선택삭제</a>
                    <a href="/Kmarket/admin/cs/faq/write.do?cate1=${cate1}&cate2=${cate2}&pg=${pg}" class="write">작성하기</a>
                    <div>
	                    <c:if test="${pageGroupStart > 1}">
	                        <a href="/Kmarket/admin/cs/faq/list.do?cate1=${cate1}&cate2=${cate2}&pg=${pageGroupStart - 1}" class="prev">이전</a>
	                    </c:if>
	                    <c:forEach var="i" begin="${pageGroupStart}" end="${pageGroupEnd}">
	                        <a href="/Kmarket/admin/cs/faq/list.do?cate1=${cate1}&cate2=${cate2}&pg=${i}" class="${currentPage == i ? 'on' : 'off'}">${i}</a>
	                    </c:forEach>
	                    <c:if test="${pageGroupEnd < lastPageNum}">    
	                        <a href="/Kmarket/admin/cs/faq/list.do?cate1=${cate1}&cate2=${cate2}&pg=${pageGroupEnd + 1}" class="next">다음</a>
	                    </c:if>
                    </div>
                </article>
            </section>
